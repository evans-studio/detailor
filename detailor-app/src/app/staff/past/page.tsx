...
export default function StaffPast() {
  ...
  const { data, loading, error, reload } = useQuery('jobs-past', () => fetchJobs('?status=completed'));
  const jobs = (data || []) as JobRow[];
  return (
    <DashboardShell role="admin" tenantName="Detailor">
      <RoleGuard allowed={["admin","staff"]}>
        <h1 className="text-[var(--font-size-2xl)] font-semibold mb-3">Past Jobs</h1>
        {loading ? (
          <div className="text-[var(--color-text-muted)]">Loading…</div>
        ) : error ? (
          <div className="text-[var(--color-danger)]">{error} <button className="underline" onClick={reload}>Retry</button></div>
        ) : jobs.length === 0 ? <div className="text-[var(--color-text-muted)]">No past jobs.</div> : (
